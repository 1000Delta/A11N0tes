...
式中：
$\boldsymbol{X} = [x_1, \ldots, x_n]^{\text{T}$ ; $p(e | \boldsymbol{X})$ 是$\boldsymbol{X}$的条件概率

\subsection{错误率分析}

\subsubsection{两类问题的错误率}%
\label{ssub:liang_lei_wen_ti_de_cuo_wu_lu_}

两类的错分

错误率计算

\subsubsection{正态分布贝叶斯决策的错误率计算}%
\label{ssub:zheng_tai_fen_bu_bei_xie_si_jue_ce_de_cuo_wu_lu_ji_suan_}

\paragraph{正态分布的对数似然比}%
\label{par:zheng_tai_fen_bu_de_dui_shu_si_ran_bi_}

\subsection{错误率的估计}

\paragraph{已设计好分类器时错误率的估计}%
\label{par:yi_she_ji_hao_fen_lei_qi_shi_cuo_wu_lu_de_gu_ji_}
...
\section{聂曼-皮尔逊(Neyman-Person)决策}
